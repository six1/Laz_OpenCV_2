...
constructor TocvCustomSourceThread.Create(CreateSuspended: Boolean; const StackSize: SizeUInt = DefaultStackSize);
begin
  inherited;
  FThreadDelay := 10;
...
{$ELSE}
        FCapture := cvCreateFileCapture(PChar(GetIPCamTarget));
{$ENDIF}
        if Assigned(FCapture) then
        begin
          (FSourceThread as TocvCaptureThread).Capture := FCapture;
