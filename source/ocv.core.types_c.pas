...
type
  ...
  {$ELSE} // D7...XE2
    TArrayOfpIplImage = array of pIplImage;//TArray<pIplImage>;
  {$ENDIF}
...
{$ENDIF}
  ...
  {$ELSE} // D7...XE2
    TArrayOfcvPoint2f = array of TcvPoint2f;
  {$ENDIF}
...
type
  ...
  TCV_TREE_NODE_FIELDS = record
    ...
    h_prev: pCvSeq;// ^node_type; (* Previous sequence. *)
    h_next: pCvSeq;//^node_type; (* Next sequence. *)
    v_prev: pCvSeq;//^node_type; (* 2nd previous sequence. *)
    v_next: pCvSeq;//^node_type; (* 2nd next sequence. *)
  end;
...
begin
  // (((elemtype*)((image)->imageData + (image)->widthStep*(row)))[(col)])
  Result := {$IFDEF DELPHI7}Pointer({$ENDIF DELPHI7}{$IFDEF FPC}Pointer({$ELSE}{$IFDEF DELPHI2005_UP}PByte{$ELSE}Integer{$ENDIF}{$ENDIF}(image^.imageData) + image^.widthStep *
    row + col * size_elemtype{$IFDEF DELPHI7}){$ENDIF DELPHI7}{$IFDEF FPC}){$ENDIF};
end;
